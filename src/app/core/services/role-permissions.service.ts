...
const DEFAULTS: RolePermission[] = [
  ...
  { key: 'manageUsers',    label: 'Crear / editar / eliminar usuarios', operator: false, guest: false },
...
export class RolePermissionsService {
  ...
  readonly operatorPerms = computed(() =>
    new Set(this._permissions().filter((p) => p.operator).map((p) => p.key))
  );

  readonly guestPerms = computed(() =>
    new Set(this._permissions().filter((p) => p.guest).map((p) => p.key))
  );
    ...
    this.ngZone.runOutsideAngular(() => {
      this.unsubscribe = onSnapshot(
        ...
        (snap) => {
          this.ngZone.run(() => {
            if (snap.exists()) {
              const data = snap.data() as { permissions: { key: string; operator: boolean; guest: boolean }[] };
              if (Array.isArray(data.permissions)) {
                ...
                const merged = DEFAULTS.map((d) => ({
                  ...d,
                  ...(saved.has(d.key) ? { operator: saved.get(d.key)!.operator, guest: saved.get(d.key)!.guest } : {}),
                }));
  ...
  private async persist(): Promise<void> {
    try {
      const ref = doc(this.firestore, DOC_PATH);
      const toSave = this._permissions().map(({ key, operator, guest }) => ({ key, operator, guest }));
